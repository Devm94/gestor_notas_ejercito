...
def notificaciones_usuario(request):
    if request.user.is_authenticated:
        ...
        notificaciones = procesamiento_nota.objects.exclude(cod_nota__cod_estado_preregistro = estado_preregistro.objects.get(id = 7)).filter(fch_limite__date=hoy)
        noti_count = procesamiento_nota.objects.exclude(cod_nota__cod_estado_preregistro = estado_preregistro.objects.get(id = 7)).filter(fch_limite__date=hoy).count() | 0 
    else:
        ...
        noti_count = f"{totalnotas_pendientes:,}"
    return {
